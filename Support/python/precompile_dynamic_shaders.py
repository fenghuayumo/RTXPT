...
def precompile_one(rtxpt, *, scene: str, shader_api: str, mode: str, frames: int) -> None:
...
def precompile(shader_api: str, scenes: list[str], modes: list[str], frames: int) -> None:
    ...
    for scene in scenes:
        for mode in modes:
            print(f"[rtxpt]   scene={scene!r}, mode={mode}")
            precompile_one(rtxpt, scene=scene, shader_api=shader_api, mode=mode, frames=frames)
