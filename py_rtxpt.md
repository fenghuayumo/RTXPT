...
- `skip_tone_mapping=True` writes pixels whose primary camera hit is this material to the background layer, bypassing auto-exposure and the tone curve. Combined with `unlit_receive_shadows=True` this puts the base color texture on screen as-is, which is how background geometry is matched to a photographed or scanned plate. The flag is ignored on transmissive/glass materials; secondary plate hits in reflections and refractions remain in the denoised, tone-mapped HDR path.
