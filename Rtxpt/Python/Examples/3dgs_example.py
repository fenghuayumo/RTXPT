...
def configure_gaussian_splats(rtxpt, settings, args: argparse.Namespace) -> None:
    ...
    settings.gaussian_splat_sorting_mode = int(
        rtxpt.GaussianSplatSortMode.StochasticSplats
        if args.sorting == "stochastic"
        else rtxpt.GaussianSplatSortMode.GpuSort
    )

    storage = {
        "float32": rtxpt.GaussianSplatStorageFormat.Float32,
        "float16": rtxpt.GaussianSplatStorageFormat.Float16,
        "uint8": rtxpt.GaussianSplatStorageFormat.Uint8,
    }[args.storage_format]
    ...
    settings.gaussian_splat_frustum_culling = int(
        {
            "disabled": rtxpt.GaussianSplatFrustumCulling.Disabled,
            "distance": rtxpt.GaussianSplatFrustumCulling.AtDistanceStage,
            "raster": rtxpt.GaussianSplatFrustumCulling.AtRasterStage,
    ...
    settings.gaussian_splat_ftb_sync_mode = int(
        rtxpt.GaussianSplatFTBSyncMode.Interlock
        if args.ftb_sync == "interlock"
        else rtxpt.GaussianSplatFTBSyncMode.Disabled
    )

    shadow_mode = {
        "disabled": rtxpt.GaussianSplatShadowMode.Disabled,
        "hard": rtxpt.GaussianSplatShadowMode.Hard,
        "soft": rtxpt.GaussianSplatShadowMode.Soft,
    }[args.shadow_mode]
...
def configure_camera(renderer, args: argparse.Namespace, ply_path: Path) -> None:
    ...
    print(f"[rtxpt] PLY vertices : {vertex_count}")
    print(f"[rtxpt] PLY center   : {center}")
    print(f"[rtxpt] PLY extents  : {extents}")
    print(f"[rtxpt] camera pos   : {cam_pos}")
    print(f"[rtxpt] camera dir   : {cam_dir}")
...
def save_or_raise(renderer, path: Path) -> None:
    ...
    print(f"[rtxpt] saved: {path}")


def render_reference_oidn(rtxpt, args: argparse.Namespace, scene: str, ply_path: Path, out_dir: Path) -> None:
    print(f"\n[rtxpt] Reference mode: accumulating {args.frames} spp with OIDN")
    renderer = make_renderer(rtxpt, args, scene, ply_path, realtime=False)
    try:
        settings = renderer.settings
        settings.path_tracer_mode = int(rtxpt.PathTracerMode.Reference)
        ...
        settings.oidn_quality = int(rtxpt.OidnQuality.High)
        settings.oidn_passes = int(rtxpt.OidnPasses.AlbedoNormal)
        settings.oidn_prefilter = int(rtxpt.OidnPrefilter.Accurate)
        settings.bounce_count = args.bounces
        settings.diffuse_bounce_count = args.diffuse_bounces
        settings.use_nee = True
        settings.oidn_apply()
        configure_gaussian_splats(rtxpt, settings, args)
        ...
        print(f"[rtxpt] reference frames executed: {frames_done} ({time.time() - start:.2f}s)")
        save_or_raise(renderer, out_dir / "reference_oidn.png")
    finally:
        renderer.close()


def select_realtime_aa(rtxpt, settings, requested: str) -> tuple[int, str]:
    ...
    if requested == "dlss-rr":
        if rr_supported:
            return int(rtxpt.RealtimeAA.DLSS_RR), "dlss_rr"
        if dlss_supported:
            print("[rtxpt] DLSS-RR unavailable; falling back to DLSS.")
            return int(rtxpt.RealtimeAA.DLSS), "dlss"
        print("[rtxpt] DLSS/DLSS-RR unavailable; falling back to TAA.")
    ...
    if requested == "taa":
        return int(rtxpt.RealtimeAA.TAA), "taa"
    return int(rtxpt.RealtimeAA.Off), "off"


def render_realtime(rtxpt, args: argparse.Namespace, scene: str, ply_path: Path, out_dir: Path) -> None:
    print(f"\n[rtxpt] Realtime mode: stepping {args.frames} frames with DLSS/DLSS-RR when available")
    renderer = make_renderer(rtxpt, args, scene, ply_path, realtime=True)
    try:
        settings = renderer.settings
        settings.path_tracer_mode = int(rtxpt.PathTracerMode.Realtime)
        aa_mode, aa_label = select_realtime_aa(rtxpt, settings, args.realtime_aa)
        ...
        print(f"[rtxpt] realtime AA: {aa_label}; frames executed: {args.frames} ({time.time() - start:.2f}s)")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RTXPT 3DGS Reference/OIDN and Realtime/DLSS example.")
...
def main() -> int:
    ...
    print(f"[rtxpt] PLY     : {ply_path}")
    print(f"[rtxpt] scene   : {scene}")
    print(f"[rtxpt] out dir : {out_dir}")
    print(f"[rtxpt] frames  : {args.frames}")

    render_reference_oidn(rtxpt, args, scene, ply_path, out_dir)
    render_realtime(rtxpt, args, scene, ply_path, out_dir)
    return 0
