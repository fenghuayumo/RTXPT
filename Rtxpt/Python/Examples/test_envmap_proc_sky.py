...
Usage:
    cd <repo>
    python Rtxpt/Python/Examples/test_envmap_proc_sky.py

    # Run only HDRI or proc-sky suites:
    python Rtxpt/Python/Examples/test_envmap_proc_sky.py --suite hdri
    python Rtxpt/Python/Examples/test_envmap_proc_sky.py --suite proc-sky

    # Single named case, interactive preview:
    python Rtxpt/Python/Examples/test_envmap_proc_sky.py --case hdri_rotation_y90 --window

    # Custom output folder and sample count:
    python Rtxpt/Python/Examples/test_envmap_proc_sky.py --out-dir ./env_tests --spp 64
"""
...
def switch_environment(renderer, case: EnvTestCase, scene_hdri_path: str) -> None:
    ...
    # against the rtxpt module directory (pip install), not this repo.
...
def render_case(
    ...
    *,
    ...
    if warmup > 0:
        print(f"[rtxpt]   warmup {warmup} frame(s) for env-map bake ...")
        renderer.step_n(warmup)

    print(f"[rtxpt]   accumulating {spp} spp ...")
    t0 = time.time()
...
def main() -> int:
    ...
    print(f"[rtxpt] Envmap / proc-sky test  ({len(cases)} case(s), {mode})")
    print(f"[rtxpt] HDRI file          : {scene_hdri_path}")
    ...
    try:
        ...
        for index, case in enumerate(cases, start=1):
            print(
                f"\n[rtxpt] Case {index}/{len(cases)}: {case.name}"
                f"  ({case.suite})"
            )
            if case.notes:
                print(f"[rtxpt]   {case.notes}")
            if case.env_source:
                print(f"[rtxpt]   env_source={case.env_source}")
            print(
                f"[rtxpt]   intensity={case.intensity}, tint={case.tint_color}, "
            ...
            if args.headless:
                ...
                if warmup > 0:
                    renderer.step_n(warmup)
                print("[rtxpt] Interactive preview ready. Close window or Ctrl+C to exit.")
    ...
    if args.headless:
        print(f"\n[rtxpt] Finished {len(cases)} case(s).")
    return 0
