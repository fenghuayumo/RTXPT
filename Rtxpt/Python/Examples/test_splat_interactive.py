#!/usr/bin/env python
"""Interactive 3DGS-only rasterization test for the RTXPT Python module.

The script launches RTXPT with an empty mesh scene and a single 3DGS .ply file.
...
def create_splat_only_scene() -> Path:
    ...
    scene_path = Path(tempfile.gettempdir()) / "rtxpt_splat_only.scene.json"
    scene = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RTXPT 3DGS-only interactive test.")
...
def main() -> int:
    ...
    print(f"[rtxpt] vertices: {vertex_count}")
    print(f"[rtxpt] center: {center}")
    print(f"[rtxpt] extents: {extents}")
    print(f"[rtxpt] camera position: {cam_pos}")
    print(f"[rtxpt] camera direction: {cam_dir}")
    ...
    try:
        ...
        print("[rtxpt] interactive: close the window or press Ctrl+C to exit.")
