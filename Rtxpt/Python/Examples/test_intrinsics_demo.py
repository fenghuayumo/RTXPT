#!/usr/bin/env python
"""Demonstrate set_camera_intrinsics issue in RTXPT.
...
REPO_ROOT = Path(__file__).resolve().parents[3] if len(Path(__file__).resolve().parents) > 3 else Path("/home/ybq/data0/Dizhou_RTXPT")


def configure_import_path() -> None:
    """Configure Python path to find rtxpt module."""
    try:
    ...
    candidates = [
        ...
        Path("/home/ybq/data0/Dizhou_RTXPT"),
    ]
...
def build_test_scene() -> str:
    ...
    This matches the approach in load_from_h5_rtxpt.py where camera is configured
    ...
    scene = {
        ...
        "graph": [
            ...
            # Note: No camera defined in scene JSON - we set it via code like load_from_h5_rtxpt.py
            {
                "name": "SampleSettings",
                "type": "SampleSettings",
                "realtimeMode": False,
...
def init_camera_from_pose(renderer, K, width, height, camera_pose):
    """Initialize camera from pose matrix and intrinsics - matches load_from_h5_rtxpt.py approach.
    
    Args:
        renderer: RTXPT renderer instance
    ...
    # Extract position and orientation from pose matrix (same as load_from_h5_rtxpt.py)
...
def build_camera_pose(position: list, look_at: list, up: list) -> 'np.ndarray':
    ...
    pose[:3, 0] = right
    pose[:3, 1] = up_new
    pose[:3, 2] = forward  # RTXPT uses pose[:3,2] directly as camera direction
    pose[:3, 3] = pos
...
def render_with_intrinsics(renderer, width: int, height: int, fx: float, fy: float, cx: float, cy: float, output_path: Path) -> bool:
    """Render using set_camera_intrinsics method - matches load_from_h5_rtxpt.py approach."""
    print(f"\n[Render] Using set_camera_intrinsics() with cx={cx:.1f}, cy={cy:.1f}")
    
    # Build camera pose matrix (same approach as load_from_h5_rtxpt.py)
    ...
    # Use the same function as load_from_h5_rtxpt.py
...
def main() -> int:
    ...
    print("=" * 60)
    print("RTXPT set_camera_intrinsics Test")
    print("=" * 60)
    ...
    try:
        ...
        s.realtime_aa = int(rtxpt.RealtimeAA.Off)
