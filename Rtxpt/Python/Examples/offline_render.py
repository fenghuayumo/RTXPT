#!/usr/bin/env python
"""
Offline / headless reference rendering using the rtxpt Python extension.

Usage:
    # Make sure rtxpt.pyd is on your PYTHONPATH (e.g. by `cd` into the bin folder
    # next to the freshly built Rtxpt.exe), then:
...
try:
    ...
    sys.stderr.write(
        "Failed to import rtxpt.  Make sure rtxpt.pyd is on PYTHONPATH:\n"
...
def render(args):
    ...
    print(f"[rtxpt] Creating Renderer ({args.width}x{args.height}, headless={args.headless}) ...")
    ...
    if args.material_overrides:
        for spec in args.material_overrides:
            ...
            if mat is not None:
                ...
                print(f"[rtxpt] WARNING: material '{name}' not found, skipping")

    # --- Render --------------------------------------------------------------
    print(f"[rtxpt] Rendering {args.spp} samples per pixel ...")
    t_start = time.time()
    frames = renderer.step_until_accumulated()
    elapsed = time.time() - t_start
    print(f"[rtxpt] Done in {elapsed:.2f}s ({frames} frames executed)")
    ...
    if renderer.save_screenshot(out_path):
        ...
        sys.stderr.write(f"[rtxpt] Failed to save screenshot to {out_path}\n")

    renderer.close()


def main():
    parser = argparse.ArgumentParser(description="RTXPT offline rendering driver.")
    ...
    parser.add_argument("--adapter-index", type=int, default=-1,
                        help="DXGI adapter index. Default -1 lets RTXPT choose.")
