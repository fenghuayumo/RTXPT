...
Usage:
    ...
    python Rtxpt/Python/Examples/render_default_scene_animated.py --window

    # Headless animation sequence:
    python Rtxpt/Python/Examples/render_default_scene_animated.py \
...
def configure_renderer(renderer, rtxpt, args: argparse.Namespace) -> tuple[object, list[tuple[float, float, float]], tuple[float, float, float]]:
    ...
    print(f"[rtxpt] Target mesh : {mesh.name} ({mesh.vertex_count} vertices)")
    print(f"[rtxpt] Deform mode : {args.deform_mode}, amplitude={args.amplitude}, speed={args.speed}")
...
def render_sequence(renderer, mesh, base_vertices, center, args: argparse.Namespace, launch_cwd: Path) -> None:
    ...
    print(f"[rtxpt] Rendering {args.frames} animation frames -> {out_dir}")
    t_start = time.time()
    ...
    print(f"[rtxpt] Animation sequence done in {elapsed:.2f}s ({args.frames} frames)")


def run_window_loop(renderer, mesh, base_vertices, center, args: argparse.Namespace) -> None:
    print("[rtxpt] Ready. Animated mesh deformation running.")
    ...
    try:
        ...
        print(f"\n[rtxpt] Interrupted after {frame} frames.")


def main() -> int:
    args = parse_args()
    if args.oidn:
        print("[rtxpt] OIDN is not supported for animated sequences; ignoring --oidn.")
        args.oidn = False
    ...
    try:
        ...
        mesh, base_vertices, center = configure_renderer(renderer, rtxpt, args)
