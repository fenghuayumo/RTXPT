...
Usage:
    cd <repo>
    python Rtxpt/Python/Examples/render_default_scene.py

    # Headless reference render (default):
    python Rtxpt/Python/Examples/render_default_scene.py --headless --out default_scene_3dgs.png

    # Interactive preview window:
    python Rtxpt/Python/Examples/render_default_scene.py --window
    python Rtxpt/Python/Examples/render_default_scene.py --no-headless
"""
...
def configure_gaussian_splats(rtxpt, settings, args: argparse.Namespace) -> None:
    ...
    if args.render_mode == "hybrid":
        ...
        settings.gaussian_splat_shadows_mode = int(rtxpt.GaussianSplatShadowMode.Soft)
        ...
        settings.gaussian_splat_shadows_mode = int(rtxpt.GaussianSplatShadowMode.Disabled)
...
def apply_gaussian_settings_and_rebuild(renderer, rtxpt, settings, args: argparse.Namespace) -> None:
    ...
    if args.render_mode == "hybrid":
        ...
        print(f"[rtxpt] Rebuilding 3DGRT acceleration structures ({warmup} warmup frames) ...")
...
def main() -> int:
    ...
    print(f"[rtxpt] 3DGS  : {render_label}, soft shadow + emitter lighting")
    ...
    try:
        ...
        print(f"[rtxpt] Materials in scene: {scene_obj.material_count}")
        print(f"[rtxpt] Lights in scene   : {scene_obj.light_count}")
        print(f"[rtxpt] 3DGS objects      : {renderer.app.gaussian_splat_object_count}")
        print(f"[rtxpt] 3DGS splat count  : {renderer.app.gaussian_splat_count}")
        ...
        apply_gaussian_settings_and_rebuild(renderer, rtxpt, settings, args)
