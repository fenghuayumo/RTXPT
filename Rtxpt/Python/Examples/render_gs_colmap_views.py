#!/usr/bin/env python
"""Render COLMAP camera views of a 3DGS PLY with the RTXPT Python module.
...
camera frame to RTXPT/Donut when the 3DGS loader uses RDF->Donut conversion,
...
def camera_params(camera: ColmapCamera) -> tuple[float, float, float, float]:
    if camera.model == "PINHOLE":
        ...
        raise ValueError(f"Unsupported camera model for RTXPT test: {camera.model}")
...
def create_splat_only_scene() -> Path:
    ...
    scene_path = Path(tempfile.gettempdir()) / "rtxpt_colmap_3dgs.scene.json"
    scene = {
...
def warn_projection_limits(view: RenderView) -> None:
    ...
    if fx_fy_delta > 1e-3 or cx_delta > 0.5 or cy_delta > 0.5:
        print(
            "[warn] RTXPT Python camera currently uses symmetric vertical FOV only; "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render COLMAP 3DGS views with RTXPT.")
...
def main() -> int:
    ...
    print(f"[rtxpt] repo       : {REPO_ROOT}")
    print(f"[rtxpt] ply        : {ply_path}")
    print(f"[rtxpt] output     : {out_dir}")
    print(f"[rtxpt] views      : {len(views)}")
    print(f"[rtxpt] resolution : {width}x{height}")
    print(f"[rtxpt] RDF->Donut : {args.convert_rdf_to_donut}")
    ...
    renderer = rtxpt.Renderer(
        ...
        gaussian_splat_convert_rdf_to_donut=args.convert_rdf_to_donut,
    ...
    try:
        ...
        settings.gaussian_splat_sorting_mode = int(rtxpt.GaussianSplatSortMode.GpuSort)
        settings.gaussian_splat_frustum_culling = int(rtxpt.GaussianSplatFrustumCulling.AtRasterStage)
        ...
        for index, view in enumerate(views):
            ...
            print(f"[rtxpt] saved {index + 1}/{len(views)}: {out_path.name}")
    ...
    print(f"[rtxpt] wrote metadata: {metadata_path}")
    return 0
