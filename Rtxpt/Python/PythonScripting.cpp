...
// Forward declared in PythonBindings.cpp - registers the rtxpt module.
extern Sample* g_pythonSampleSingleton;

// Symbol defined by NB_MODULE(rtxpt, m) inside PythonBindings.cpp - we declare
// it manually here so we can hand it to PyImport_AppendInittab without pulling
// nanobind into the call site.
extern "C" PyObject* PyInit_rtxpt(void);
...
if not hasattr(sys, "_rtxpt_capture_stdout"):
    sys._rtxpt_capture_stdout = _RtxptStdoutCapture("stdout")
    sys._rtxpt_capture_stderr = _RtxptStdoutCapture("stderr")
    sys.stdout = sys._rtxpt_capture_stdout
    sys.stderr = sys._rtxpt_capture_stderr
)PY";
}
...
bool PythonScripting::Initialize()
{
    ...
    // immediately `import rtxpt` without touching sys.path.
    if (PyImport_AppendInittab("rtxpt", PyInit_rtxpt) != 0)
    {
    ...
    // Hook up stdout/stderr redirection so script output ends up in our log.
    {
        ...
        try
        {
            ...
            // Pre-import rtxpt so the user does not need to do it explicitly
            // for inline expressions (interactive console, etc.).
            PyRun_SimpleString("import rtxpt as _rtxpt_preimported");
        }
...
bool PythonScripting::RunPendingLocked(const PendingScript& script)
{
    ...
    try
    {
        ...
        for (const char* attr : { "_rtxpt_capture_stdout", "_rtxpt_capture_stderr" })
        {
