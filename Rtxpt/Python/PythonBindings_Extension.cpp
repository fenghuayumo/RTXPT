...
// (rtxpt.pyd / rtxpt.so) so that standalone Python interpreters can drive
// RTXPT for offline rendering and headless data generation.
//
//    python -c "import rtxpt; r = rtxpt.Renderer(headless=True); ..."
...
namespace
{
    ...
    // helpers like `rtxpt.app()` and `rtxpt.settings()` keep working.
    Sample* g_currentExtensionSample = nullptr;

    Sample& RequireCurrentSample()
    {
        if (!g_currentExtensionSample)
            throw std::runtime_error("rtxpt: no Renderer is currently active. Create one via rtxpt.Renderer(...)");
...
public:
               ...
               float gaussianSplatAlphaCullThreshold)
    {
        ...
        if (!m_owned)
            throw std::runtime_error("rtxpt.Renderer: failed to initialize the renderer (see log for details)");
...
NB_MODULE(rtxpt, m)
{
    m.doc() = "RTXPT Python extension - drive the path-tracer for offline rendering.";
