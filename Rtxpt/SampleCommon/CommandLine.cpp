...
bool CommandLineOptions::InitFromCommandLine(int _argc, char const* const* _argv)
{
	...
	try
	{
		...
		options.add_options()
			...
			("a,adapter", "GPU to render on, given as a case-insensitive part of its name or as an index, e.g '--adapter RTX' or '--adapter 1'; by default the adapter with the best expected compute performance is used", value(adapter))
            ...
            ("listAdapters", "Print the GPUs available for rendering, mark the one that would be selected, and exit", value(listAdapters))
