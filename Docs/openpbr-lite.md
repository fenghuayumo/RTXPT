...
| `SkipToneMapping` | Pixels whose primary camera hit is this material bypass auto-exposure and the tone curve through the background layer. Pairs with `UnlitReceiveShadows` to match photographic plates. Ignored on transmissive/glass materials; secondary plate hits in reflections and refractions remain in the denoised, tone-mapped HDR path. |
