...
| `SkipToneMapping` | Primary-hit pixels bypass auto-exposure and the tone curve, so the linear result reaches the sRGB backbuffer unaltered. Pairs with `UnlitReceiveShadows` to match photographic plates. |
